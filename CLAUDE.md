...
- Don't: access `db.json` outside `server/src/repositories/` — routes and services never import `db/store.ts` (exceptions: `server/src/index.ts`, the composition root, and type-only imports). Enforced by a `PreToolUse` block (`.claude/hooks/check-repo-boundary.js`) and the `repo-boundary` CI job — both **fail** rather than nudge, since this is an unambiguous architectural boundary, not a heuristic. Bypass: a maintainer applies the `skip-governance` label with a reason in the PR description
